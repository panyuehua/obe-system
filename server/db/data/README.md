...
每个专业按届次维护培养方案；同一届次可有多个修订版本（版本号不同），**UNIQUE 约束为 `major_id + grade_year + version`**。从已有方案派生新版本时，应调用 **`POST /api/curriculum/versions/:id/clone`**，在服务端事务中整包复制培养目标、毕业要求、指标点、课程、先修关系、目标↔毕业要求支撑、指标↔课程支撑及改进计划（不复制教学班、成绩等运行数据）。
