/**
 * 专业管理 — majors.html
 */
nav.render('majors');

let allMajors = [];
let allColleges = [];
const certMap = { vocational:'高职专科', engineering:'工程认证', other:'其他' };
const certColor = { vocational:'badge-primary', engineering:'badge-info', other:'badge-neutral' };

async function loadMajors() {
  try {
    [allMajors, allColleges] = await Promise.all([api.majors.list(), api.colleges.list()]);
    renderGrid(allMajors);
  } catch (e) { toast.error('加载失败：' + e.message); }
}

function filterMajors() {
  const q = document.getElementById('search-input').value.toLowerCase();
  renderGrid(allMajors.filter(m => m.name.toLowerCase().includes(q) || (m.code||'').toLowerCase().includes(q)));
}

function renderGrid(list) {
  document.getElementById('count-label').textContent = `共 ${list.length} 个专业`;
  const grid = document.getElementById('majors-grid');
  if (!list.length) {
    grid.innerHTML = `
      <div class="col-span-3">
        <div class="card">
          <div class="empty-state">
            <div class="empty-state-icon">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg>
            </div>
            <p class="empty-state-title">暂无专业</p>
            <p class="empty-state-desc">点击右上角「添加专业」创建第一个专业，或从教务系统同步数据。</p>
            <button onclick="openCreateModal()" class="btn btn-primary btn-sm mt-2">添加专业</button>
          </div>
        </div>
      </div>`;
    return;
  }
  grid.innerHTML = list.map((m) => `
    <div class="card card-hover">
      <div class="card-body">
        <div class="flex items-start justify-between mb-4">
          <div style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,var(--color-primary-500),var(--color-primary-700));display:flex;align-items:center;justify-content:center;flex-shrink:0">
            <span style="color:#fff;font-size:13px;font-weight:700">${m.code ? m.code.slice(0,2).toUpperCase() : m.name.slice(0,1)}</span>
          </div>
          <span class="badge ${certColor[m.cert_type]||'badge-neutral'}">${certMap[m.cert_type]||m.cert_type}</span>
        </div>

        <h3 style="font-size:15px;font-weight:700;color:var(--color-neutral-900);margin-bottom:4px">${m.name}</h3>
        <div class="flex items-center gap-3 mb-4">
          <span style="font-size:12px;color:var(--color-neutral-400)">${m.college_name}</span>
          <span style="width:3px;height:3px;border-radius:50%;background:var(--color-neutral-300)"></span>
          <span style="font-size:12px;color:var(--color-neutral-400)">${m.degree_years} 年制</span>
          ${m.code ? `<span style="width:3px;height:3px;border-radius:50%;background:var(--color-neutral-300)"></span><span style="font-size:12px;color:var(--color-neutral-400);font-family:monospace">${m.code}</span>` : ''}
        </div>

        <div class="flex items-center gap-2 pt-3" style="border-top:1px solid var(--color-border-subtle)">
          <div style="flex:1;display:flex;align-items:center;gap:4px">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--color-neutral-400)"><rect x="9" y="2" width="6" height="4" rx="1"/><path d="M16 4h2a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2"/></svg>
            <span style="font-size:12px;color:var(--color-neutral-500)">${m.version_count} 个培养方案版本</span>
          </div>
          <a href="/pages/curriculum.html?major_id=${m.id}" class="btn btn-secondary btn-sm">培养方案</a>
          <button onclick='openEditModal(${JSON.stringify(m).replace(/'/g,"&#39;")})' class="btn btn-ghost btn-sm" aria-label="编辑">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          </button>
          <button onclick="deleteMajor(${m.id},'${String(m.name).replace(/'/g, "\\'")}')" class="btn btn-ghost btn-sm" aria-label="删除" style="color:var(--color-error)">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg>
          </button>
        </div>
      </div>
    </div>
  `).join('');
}

const formFields = (d = {}) => [
  { name:'name',         label:'专业名称',   required:true, placeholder:'例如：软件技术', default:d.name },
  { name:'code',         label:'专业代码',   placeholder:'如 RJ',              default:d.code },
  { name:'college_id',   label:'所属学院',   type:'select', required:true,
    options: allColleges.map(c => ({ value: c.id, label: c.name })),
    default: d.college_id || (allColleges[0] && allColleges[0].id) || '' },
  { name:'degree_years', label:'学制（年）', type:'number',                       default:d.degree_years||3 },
  { name:'cert_type',    label:'认证类型',   type:'select',
    options:[{value:'vocational',label:'高职专科'},{value:'engineering',label:'工程认证'},{value:'other',label:'其他'}],
    default:d.cert_type||'vocational' },
];

function openCreateModal() {
  modal.form({ title:'添加专业', fields:formFields(), submitText:'创建',
    onSubmit: async (data, close) => {
      try {
        await api.majors.create({ ...data, college_id: Number(data.college_id), degree_years: Number(data.degree_years) });
        close(); toast.success('专业已创建'); loadMajors();
      } catch(e) { toast.error(e.message); }
    }
  });
}

function openEditModal(m) {
  modal.form({ title:'编辑专业', fields:formFields(m), submitText:'保存',
    onSubmit: async (data, close) => {
      try {
        await api.majors.update(m.id, { ...data, college_id: Number(data.college_id), degree_years: Number(data.degree_years) });
        close(); toast.success('已保存'); loadMajors();
      } catch(e) { toast.error(e.message); }
    }
  });
}

async function deleteMajor(id, name) {
  const ok = await modal.confirm(`确定要删除专业 <strong>${name}</strong> 吗？<br><br>若该专业下存在培养方案版本，请先处理或确认可一并清理相关数据。`, { title:'删除确认', danger:true });
  if (!ok) return;
  try { await api.majors.remove(id); toast.success('已删除'); loadMajors(); }
  catch(e) { toast.error(e.message); }
}

loadMajors();
